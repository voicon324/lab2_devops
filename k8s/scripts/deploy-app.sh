...
deploy_gateway() {
    ...
    echo "Waiting for API Gateway pod to be created..."
    sleep 5  # Give Kubernetes time to create the pod
    
    # Wait for pod to exist first
    for i in {1..30}; do
        if kubectl get pods -l app=api-gateway -n petclinic 2>/dev/null | grep -q api-gateway; then
            echo "API Gateway pod found, waiting for it to be ready..."
            break
        fi
    ...
    kubectl wait --for=condition=ready pod -l app=api-gateway -n petclinic --timeout=300s || {
        echo -e "${RED}Warning: API Gateway pod not ready yet. Checking status...${NC}"
        kubectl get pods -l app=api-gateway -n petclinic
        kubectl describe pod -l app=api-gateway -n petclinic | tail -20
    }
...
show_access_info() {
    ...
    if [ "$GATEWAY_IP" == "pending" ] || [ -z "$GATEWAY_IP" ]; then
        ...
        echo "  kubectl port-forward svc/api-gateway -n petclinic 9090:8080"
